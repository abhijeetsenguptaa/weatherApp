...
const { emailChecker } = require('../middleware/emailChecker.middleware');
...
// This is the route made for the user-registration----------------------------
userRoute.post('/register', emailChecker, async (req, res) => {
    try {
        ...
        res.send({ 'msg': 'Error in registering the New User', 'err': err })
        logger.error({ 'msg': 'Error in registering the New User', 'err': err })
    }
})

// This is route made for the user-login---------------------------------------
        ...
        if (userData.length >= 1) {
            bcrypt.compare(password, userData[0].password, async (err, result) => {
                if (result) {
                    ...
                    // await client.set(`${userData[0].email}`,token)
...
// This is the route which is made for logging-out a user.
